...
### PAYMENT_PENDING — checkout em aberto

Quando o atleta abre `/pagamento/{id}` **sem** o parâmetro `?status=...` (por exemplo, fechou a aba durante o checkout externo e voltou mais tarde), o frontend detecta se há um pagamento de cartão com `checkoutUrl` ativo em `AWAITING_PAYMENT` e entra no estado `PAYMENT_PENDING`:
...
<Info>
Ao clicar em "Trocar método de pagamento" a partir de `PAYMENT_PENDING`, a seleção de método (cartão) é mantida — apenas o estado da tela muda para SELECT. O atleta pode então escolher PIX ou confirmar novo checkout de cartão.
</Info>

### CARD_CONFIRMING via internalStatus PROCESSING

Além do retorno via `?status=paid`, o estado `CARD_CONFIRMING` também é ativado quando o polling retorna `internalStatus = PROCESSING`. Isso cobre o caso em que o gateway está processando o pagamento (ex: análise antifraude em andamento) sem ter emitido o webhook de confirmação ainda.
...
| `CANCELLED` | Sobreescreve o status do gateway — exibe tela de cancelado |
| `PROCESSING` | Frontend entra em `CARD_CONFIRMING` — "Confirmando pagamento..." |
...
O campo `obs` é um texto livre opcional registrado pelo backend em transições de estado relevantes (especialmente `CANCELLED`, `PROCESSING`, `ERROR`). Quando presente, é retornado no payload de `initiatePayment` / `getOrInitiatePaymentByToken` e exibido na tela **"Pagamento não confirmado"** como motivo para o atleta.

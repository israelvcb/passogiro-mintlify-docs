...
flowchart TD
  ...
  CHECK -->|PAID / CATEGORY_CHANGED| SUCCESS["PIX_PAID\n✅ Sucesso com comprovante"]
  ...
  QR -->|Polling EXPIRED / timer| EXP["PIX_EXPIRED\n⏰ QR expirado"]
  ...
  CF -->|Polling CANCELLED| EXP2["PIX_EXPIRED\n❌ Pagamento não confirmado"]
...
- Polling retorna `PAID` → `PIX_PAID`
- Polling retorna `EXPIRED` / `CANCELLED` → `PIX_EXPIRED` ("Código PIX expirado")
- Polling retorna `REGISTRATION_EXPIRED` / `REGISTRATION_CANCELLED` / `REGISTRATION_REFUNDED` → `PixErrorView` correspondente
- Timer de expiração chega a zero → `PIX_EXPIRED`
...
- Polling retorna `CANCELLED` → `PIX_EXPIRED` ("Pagamento não confirmado")
...
**Transições automáticas:**
- Polling retorna `PAID` → `PIX_PAID`
- Polling retorna `CANCELLED` ou `EXPIRED` com `paymentMethod = CREDIT_CARD` → `PIX_EXPIRED` ("Pagamento não confirmado")
...
### PIX_PAID — Pagamento confirmado (sucesso)
...
### PIX_EXPIRED — QR expirado ou pagamento recusado
...
<Note>
Variante especial: quando a cortesia é detectada via `REGISTRATION_ALREADY_PAID` no modo público, o sistema carrega e exibe o **comprovante completo** (tela `PIX_PAID`) em vez desta tela de bloqueio.
</Note>
...
| PIX_PAID | ✅ | ✅ | Qualquer pagamento confirmado |
| PIX_EXPIRED (QR) | ✅ | ❌ | PIX expirou ou cancelado |
| PIX_EXPIRED (Cartão) | ❌ | ✅ | Cartão recusado |
